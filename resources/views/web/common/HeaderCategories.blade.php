  ...
  $option = '';
    ...
    foreach($categories as $parents){
      ...
      $option .= '<a class="dropdown-item categories-list '.$selected.'" value="'.$parents->categories_name.'" slug="'.$parents->slug.'" '.$selected.'>'.$parents->categories_name.'</a>';
       ...
       $option .= '<a class="list-group-item border-default  '.$selected.'" value="'.$parents->categories_name.'" slug="'.$parents->slug.'" '.$selected.' data-toggle="tab" href="#mega-menu-tab-dog" role="tab" aria-controls="mega-menu-tab-dog" aria-selected="true"><img width="40px" src="asset()" alt="Image" class="rounded-circle">'.$parents->categories_name.'</a>';
