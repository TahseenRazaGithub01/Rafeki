...
///////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function supplierProduct($category_slug, Request $request)
    {
        ...
        if($request->ajax()){
            ...
            $vendor_name = $urlArray[6];
            $vendor_id = DB::table('users')->select('id')->where('user_name', $vendor_name)->pluck('id');
        ...
        $categories_status = 1;
         ...
         if($request->ajax() && $request->get('category_slug')){

               $result['category'] =   $result['category']->whereIn('categories_slug', $one);
    ...
    // Vendor products
        public function vendorProducts($data)
    {
        // dd($data) ;
        if (empty($data['page_number']) or $data['page_number'] == 0) {
            $skip = $data['page_number'] . '0';
        ...
        $categories = DB::table('products')
        ->where('products.user_id', $data['vendor_id'])
            ->leftJoin('manufacturers', 'manufacturers.manufacturers_id', '=', 'products.manufacturers_id')
            ->leftJoin('manufacturers_info', 'manufacturers.manufacturers_id', '=', 'manufacturers_info.manufacturers_id')
            ->leftJoin('products_description', 'products_description.products_id', '=', 'products.products_id')
            ->LeftJoin('image_categories', 'products.products_image', '=', 'image_categories.image_id');
            //->LeftJoin('categories','categories.categories_id','=','products_to_categories.products_id');


        if (!empty($data['filterCatIds']) ) {
            // dd('sad');
            $categories
            ->LeftJoin('products_to_categories as ptc', 'products.products_id', '=', 'ptc.products_id')
               
                ->whereIn('ptc.categories_id', $data['filterCatIds']);
