...
class Customers extends Model
{
    ...
    public function insertVendor($request, $vendor_image, $vendor_banner_image){
      $uploadImage = '';
    ...
    if( $vendor_image != NULL){
      ...
      DB::table('vendor_details')->where('id','=', $vendor_detail_id)->update([
              'vendor_logo'     =>   $vendor_image
          ]);
    
    }
    
    if( $vendor_banner_image != NULL){
      
      DB::table('vendor_details')->where('id','=', $vendor_detail_id)->update([
              'vendor_banner_image'     =>   $vendor_banner_image
          ]);
    ...
    public function addresses($id){
        $addresses = DB::table('user_to_address')
            ...
            ->leftJoin('zones', 'zones.zone_id', '=', 'address_book.entry_zone_id')
            ->leftJoin('countries', 'countries.countries_id', '=', 'address_book.entry_country_id')
      //->leftJoin('vendor_details', 'user_to_address.user_id','=','vendor_details.vendor_id')
            ->where('user_to_address.user_id', '=', $id)->get();  
          return $addresses;
    }
