...
class CustomersController extends Controller
{
    ...
    public function login(Request $request)
    {
     ...
     public function vendorLogin(Request $request){
        ...
        if (auth()->guard('customer')->check()) {
            ...
            $final_theme = $this->theme->theme();
            $result['commonContent'] = $this->index->commonContent();
            return view("auth.vendor_login", ['title' => $title, 'final_theme' => $final_theme])->with('result', $result);
        }
    ...
    public function signupProcess(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if(isset($request->vendor_login) > 0){
            ...
            $checkStoreName = DB::table('users')->select('user_name')->where('user_name', $store_name)->get();

            if( count($checkStoreName) > 0){
                return redirect('/vendor-login')->withInput($request->input())->with('error', Lang::get("website.storeNameAlreadyExist"));
            }
