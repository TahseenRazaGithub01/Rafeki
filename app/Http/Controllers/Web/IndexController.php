...
//current time
        ...
        // $home_page_show_vendors = DB::table('vendor_details')->select('vendor_id','store_slug','vendor_logo')->orderBy('id','DESC')->take(7)->get();
        // $result['home_page_show_vendors'] = $home_page_show_vendors;

        $home_page_show_vendors = DB::table('users')->select('users.id','users.user_name','vd.vendor_logo')->where('role_id', 14)->where('users.status','=','1')->orderBy('users.id','DESC')
        ->join('vendor_details as vd','users.id','=','vd.vendor_id')
        ->take(7)->get();
        $result['home_page_show_vendors'] = $home_page_show_vendors;

        // take 7 and order by id desc

        // dd( $home_page_show_vendors );
         // return $result;
...
//current time
        ...
        return view("web.index_orignel", ['title' => $title, 'final_theme' => $final_theme])->with(['result' => $result])->with('currencies',$currencies);
    }
